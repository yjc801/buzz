  ...
  dom.window.__TAURI_INTERNALS__ = {
    invoke: (command, args) => {
      ...
      if (command === "start_managed_agent") {
        ...
        // Fork-local return shape: `start_managed_agent` resolves to a
        // StartManagedAgentOutcome, not a bare record — the provider's
        // fresh-generation classification rides alongside the agent.
        const started = {
          agent: { pubkey: args.pubkey, status: "running" },
