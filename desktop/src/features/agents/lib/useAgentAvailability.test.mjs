...
for (const presence of ["online", "away", "offline", undefined]) {
  test(`retained deployment receipt does not supply availability (${presence})`, () => {
    ...
    // Fork-local: for a provider agent the primary label routes on LIVENESS,
    // not on the retained receipt. `backendAgentId` proves infrastructure was
    // created, never that a harness is running, so a dead remote agent offers
    // Deploy and only a present one offers Shutdown. Upstream routes this on
    // `isManagedAgentActive` alone and so expects "Shutdown" unconditionally.
    assert.equal(
