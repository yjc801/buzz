...
const agent = {
  ...
  // Fork-only record field: `deleteManagedAgentWithRules` confirms separately
  // for deployments the agent has moved off, and reads this list directly.
  residualDeployments: [],
};
...
test("Agents deletion rechecks availability after channel discovery, not the click-time snapshot", async () => {
  ...
  assert.deepEqual(effects(), []);
  await act(async () => {
    // The discovered list must contain the agent's channel: this fork's
    // `resolveManagedAgentChannelId` only addresses an id that resolves to a
    // known, unarchived channel (an id the caller cannot see is one it cannot
    // write to), so an empty list would take the "not in any channel" path and
    // stop testing the availability recheck this case is about.
    releaseChannels({
      hash: "discovered",
      channels: [channel],
      last_messages: {},
