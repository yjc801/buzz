...
export function useCanonicalManagedAgentProfile(input: {
  ...
  // Upstream #7131 made an explicit pubkey resolve to exactly that identity, so
  // this pin no longer affects resolution (it is deliberately NOT passed to
  // `resolveCanonicalManagedAgent`). It survives only as a "the user
  // deliberately opened this instance" signal: the panel reads it to keep the
  // Runtime tab it just set instead of resetting the view on target change.
