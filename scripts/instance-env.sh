...
BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"xyz.waggle.app.dev\",\"productName\":\"Buzz Dev\"}"
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    if [[ -n "$GIT_COMMON_DIR" && "$GIT_DIR" != "$GIT_COMMON_DIR" ]]; then
        ...
        if [[ "${BUZZ_SHARE_IDENTITY:-0}" == "1" ]]; then
            ...
            CANONICAL_KEY="$HOME/Library/Application Support/xyz.waggle.app.dev/identity.key"
            LEGACY_BUZZ_KEY="$HOME/Library/Application Support/xyz.block.buzz.app.dev/identity.key"
            LEGACY_SPROUT_KEY="$HOME/Library/Application Support/xyz.block.sprout.app.dev/identity.key"

            SHARED_IDENTITY="$KEYRING_IDENTITY"
            if [[ -z "$SHARED_IDENTITY" && -f "$CANONICAL_KEY" ]]; then
                SHARED_IDENTITY="$(cat "$CANONICAL_KEY")"
            elif [[ -z "$SHARED_IDENTITY" && -f "$LEGACY_BUZZ_KEY" ]]; then
                SHARED_IDENTITY="$(cat "$LEGACY_BUZZ_KEY")"
            elif [[ -z "$SHARED_IDENTITY" && -f "$LEGACY_SPROUT_KEY" ]]; then
                SHARED_IDENTITY="$(cat "$LEGACY_SPROUT_KEY")"
            fi

            if [[ -n "$SHARED_IDENTITY" ]]; then
                export BUZZ_PRIVATE_KEY="$SHARED_IDENTITY"
            else
                echo "⚠ BUZZ_SHARE_IDENTITY=1 but no identity found in keyring service $KEYRING_SERVICE, at $CANONICAL_KEY, at $LEGACY_BUZZ_KEY, or at $LEGACY_SPROUT_KEY — run Buzz from repo root first" >&2
            fi
        fi
        ...
        if swift "$GENERATE_DEV_ICON" "$BASE_ICON" "$DEV_ICON" "$BUZZ_WORKTREE_LABEL"; then
            ...
            BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"xyz.waggle.app.dev.${BUZZ_INSTANCE_SLUG}\",\"productName\":\"Buzz Dev (${BUZZ_WORKTREE_LABEL})\",\"bundle\":{\"icon\":[\"$DEV_ICON\"]}}"
        fi
    fi
fi
