...
function renderSection(props) {
  ...
  return render(
    createElement(
      ...
      // assertions do not touch. `Surface` sits inside it because it supplies
      // the getAvailability prop the section now requires.
...
test("N cards share a snapshot, one poll, failure recovery and live subscription lifecycle", async (t) => {
  ...
  const tree = (next) =>
    createElement(
      QueryClientProvider,
      { client },
      // Same provider requirement as `renderSection`: the instance card
      // renders AgentCommunityScopeBadge, which throws outside a
      // CommunitiesProvider.
      createElement(
        CommunitiesProvider,
        null,
