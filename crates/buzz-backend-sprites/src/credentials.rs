...
fn resolve_with(
    ...
    if let Some(service) = keyring_service(home, org_override) {
        if let Some(value) = lookup(&service) {
            let trimmed = value.trim();
            if !trimmed.is_empty() {
                return Ok(Credential {
                    token: trimmed.to_string(),
...
/// Walk the sprite CLI's metadata to the keychain service name for the
/// selected org: `~/.sprites/sprites.json` → `current_user` + selection →
/// `~/.sprites/users/<id>.json` → `urls.<url>.orgs.<org>.keyring_key`.
///
...
fn keyring_service(home: &Path, org_override: Option<&str>) -> Option<String> {
    ...
    // Newer split layout: per-user config file named by `current_user`.
    let user_file = if let Some(user_id) = root.get("current_user").and_then(|u| u.as_str()) {
        let user: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(home.join(".sprites/users").join(format!("{user_id}.json")))
                .ok()?,
        )
        .ok()?;
        Some(user)
...
/// Read a generic password from the macOS keychain by service name, via the
/// absolute `security` path (this process may run with launchd's minimal
/// PATH). First use triggers the OS's one-time Allow prompt attributed to
/// this binary; the deploy budget (600s) absorbs the wait. A denial or a
/// missing entry both read as `None`.
...
mod tests {
    ...
    /// Build a `~/.sprites` tree in the newer split layout.
    fn sprites_home(dir: &Path, org: &str, keyring_key: &str) {
        ...
        .unwrap();
        std::fs::write(
            dir.join(".sprites/users/user-1.json"),
            serde_json::json!({
    ...
    fn keychain_arm_uses_the_cli_metadata() {
        ...
        sprites_home(&dir, "my-org", "sprites-cli:user-1");

        let cred = resolve_with(
            no_env,
            &dir,
            None,
            |service| (service == "sprites-cli:user-1").then(|| "tok-k\n".into()),
        )
        .unwrap();
        ...
        let err = resolve_with(no_env, &dir, Some("other-org"), |service| {
            (service == "sprites-cli:user-1").then(|| "tok-k".into())
        })
        .unwrap_err();
