...
export function MembersSidebar({
  ...
  // The query itself is still needed for the resolved/unresolved distinction:
  // `getAvailability` collapses "not yet loaded" and "resolved offline" to
  // different values, but only the query says whether it has settled at all.
  const { getAvailability, query: memberPresenceQuery } =
    useAgentAvailabilityLookup(allMemberPubkeys, {
  ...
  function renderMemberCard(member: ChannelMember, memberIsBot: boolean) {
    ...
    const pairAction =
      managedAgent?.backend.type === "local" && relayUrl
        ...
        : undefined;
    const presenceStatus = getAvailability(member.pubkey);
