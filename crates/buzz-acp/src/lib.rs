...
mod replay_floor_tests {
    ...
    fn the_budget_covers_every_enforced_bound_stacked() {
        // Worst legitimate case: max relay past skew, then the full evidence
        // window, the provider info probe, and the full deploy invocation
        // timeout back to back — the shared budget is the sum of those
        // enforced bounds plus margin, so the trigger survives instead of
        // being clamped away. A local 15-minute literal fails this.
