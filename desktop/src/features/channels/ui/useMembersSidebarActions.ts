...
export function useMembersSidebarActions({
  ...
  async function handleRespawnAll() {
    // Bounded parallelism, unlike the serial stop/remove bulks (and unlike
    // `runBulkAgentAction`, which awaits each agent in turn): every live
    ...
    // per-agent because `mapWithConcurrency` captures each throw.
    clearActionFeedback();
    ...
    try {
      const results = await mapWithConcurrency(
        ...
        async (agent) => {
          // Same presence gate the single-agent and other bulk paths apply:
          // positive relay presence means a body is already up, so starting
          // another is refused rather than silently doubled.
